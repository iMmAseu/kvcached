...
Config parse_args(int argc, char **argv) {
  Config cfg;
  cfg.pages_list = parse_pages_list("1,2,4,8,12,16,20,24,32,48,64");
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--pages-list") {
      ...
      std::cout
          << "Usage: bench_vmm_unmap_compare.bin [options]\n"
          << "  --pages-list 1,2,4,...       Comma-separated pages per unmap event\n"
...
double run_loop_unmap_once(int pages, size_t page_size,
                           const CUmemAllocationProp &prop,
                           const CUmemAccessDesc &access_desc) {
  const size_t total_bytes = static_cast<size_t>(pages) * page_size;
  CUdeviceptr addr = 0;
  CHECK_DRV(cuMemAddressReserve(&addr, total_bytes, page_size, 0, 0));

  std::vector<CUmemGenericAllocationHandle> handles(pages);
  for (int i = 0; i < pages; ++i) {
    CHECK_DRV(cuMemCreate(&handles[i], page_size, &prop, 0));
    const auto page_addr = addr + static_cast<CUdeviceptr>(i) * page_size;
    CHECK_DRV(cuMemMap(page_addr, page_size, 0, handles[i], 0));
    CHECK_DRV(cuMemSetAccess(page_addr, page_size, &access_desc, 1));
  }
  ...
  for (int i = 0; i < pages; ++i) {
    const auto page_addr = addr + static_cast<CUdeviceptr>(i) * page_size;
    CHECK_DRV(cuMemUnmap(page_addr, page_size));
  }
  CHECK_RT(cudaDeviceSynchronize());
  ...
  CHECK_DRV(cuMemAddressFree(addr, total_bytes));

  return std::chrono::duration<double, std::milli>(t1 - t0).count();
}

double run_range_unmap_once(int pages, size_t page_size,
                            const CUmemAllocationProp &prop,
                            const CUmemAccessDesc &access_desc) {
  const size_t total_bytes = static_cast<size_t>(pages) * page_size;
  CUdeviceptr addr = 0;
  ...
  CHECK_DRV(cuMemCreate(&handle, total_bytes, &prop, 0));
  CHECK_DRV(cuMemMap(addr, total_bytes, 0, handle, 0));
  CHECK_DRV(cuMemSetAccess(addr, total_bytes, &access_desc, 1));

  CHECK_RT(cudaDeviceSynchronize());
  const auto t0 = std::chrono::high_resolution_clock::now();
  CHECK_DRV(cuMemUnmap(addr, total_bytes));
  CHECK_RT(cudaDeviceSynchronize());
  const auto t1 = std::chrono::high_resolution_clock::now();
...
void print_result_line(const char *mode, int pages,
  ...
  std::cout << "[result][" << mode << "] pages=" << std::setw(3) << pages
            ...
            << " batch_unmap_bytes=" << std::setprecision(2) << batch_unmap_mb
            << "MB" << std::endl;
}
...
int main(int argc, char **argv) {
  try {
    ...
    std::cout << "[setup] compare looped page-wise unmap vs contiguous-range "
                 "cuMemUnmap(ptr, size)"
              << std::endl;
    std::cout << "[setup] page_size=" << (cfg.page_size >> 20)
              << "MB repeats=" << cfg.repeats
              << " warmup_repeats=" << cfg.warmup_repeats << std::endl;
    std::cout << "[setup] free_memory_gb=" << std::fixed
              << std::setprecision(2)
              << (static_cast<double>(free_bytes) / std::giga::num)
              << std::endl;
    ...
    for (const int pages : cfg.pages_list) {
      std::vector<double> loop_latencies_ms;
      std::vector<double> range_latencies_ms;
      loop_latencies_ms.reserve(static_cast<size_t>(cfg.repeats));
      range_latencies_ms.reserve(static_cast<size_t>(cfg.repeats));

      const int total_runs = cfg.warmup_repeats + cfg.repeats;
      for (int rep = 0; rep < total_runs; ++rep) {
        const double loop_ms =
            run_loop_unmap_once(pages, cfg.page_size, prop, access_desc);
        const double range_ms =
            run_range_unmap_once(pages, cfg.page_size, prop, access_desc);
        if (rep >= cfg.warmup_repeats) {
          loop_latencies_ms.push_back(loop_ms);
          range_latencies_ms.push_back(range_ms);
        }
      }

      print_result_line("loop", pages, loop_latencies_ms, cfg.page_size);
      print_result_line("range", pages, range_latencies_ms, cfg.page_size);
    }
